...
const CampaignManagement: React.FC<{ user: any }> = ({ user }) => {
  ...
  const getStatusStyle = (status: CampaignStatus) => {
    switch (status) {
      ...
      case CampaignStatus.MANAGER_APPROVED:
        return 'bg-indigo-50 text-indigo-600 border-indigo-100';
      case CampaignStatus.COMPLIANCE_APPROVED:
          ...
          <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <input type="text" className="w-full bg-slate-50 border-none rounded-xl p-3 focus:ring-2 focus:ring-blue-500" placeholder="例如：某药临床研讨会" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input type="number" className="w-full bg-slate-50 border-none rounded-xl p-3 focus:ring-2 focus:ring-blue-500" placeholder="金额" />
                </div>
              </div>
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-1">活动描述</label>
                <textarea className="w-full bg-slate-50 border-none rounded-xl p-3 focus:ring-2 focus:ring-blue-500 h-24" placeholder="请简述活动目的与形式..."></textarea>
              </div>
            </div>
            <div className="p-6 bg-slate-50 flex items-center justify-end space-x-3">
              ...
              <button className="px-6 py-2.5 bg-blue-600 text-white rounded-xl font-bold shadow-lg shadow-blue-200 hover:bg-blue-700 transition-all">提交审批</button>
            </div>
          </div>
        </div>
      )}
